...
class IwBot(GenBot):
    ...
    def __init__(self, locargs = []):
        ...
        self.titleExceptions = ['Користувач:',
                                'Вікіпедія:Кнайпа',
                                'Обговорення:',
    ...
    def run(self):
                ...
                for page in self.generator:
                    ...
                    if self.getOption('notsafe'):
                        ...
                        try:
                            ...
                            self.addProblem(page, 'Unexpected error (%s) occured while processing page \03{blue}[[%s]]\03{default}!' % (e, page.title()))
            except KeyboardInterrupt:
                raise KeyboardInterrupt
        ...
        if self.getOption('report'):
            ...
            pywikibot.output('%d pages were not changed' % (self.Ntotal - self._save_counter))
    ...
    def treat(self, page):
        ...
        if self.ok and self.getOption('replace'):
            ...
            pywikibot.output('Page [[%s]] was not changed because of the above problems' % page.title())
            pywikibot.output('=' * 80)
        elif not self.ok:
    ...
    def iwanalyze(self, page, pageText, iw):
        ...
        try:
            ...
            self.addProblem(page, 'Page contains bare \03{blue}{{tl|Не перекладено}}\03{default} without parameters!')
            return
            
        # Find, whether the page was translated
        if not mova in wikicodes:
            self.addProblem(page, 'Language code \03{blue}"%s"\03{default} is not supported!' % mova)
            return
        ...
        if mova == 'd':
            try:
                ...
                self.addProblem(page, 'Data item \03{blue}[[:%s:%s]]\03{default} does not exist!' % (mova, ee))
                return
        else:
            ...
            try:
                if eePage.exists():
                    ...
                    self.addProblem(page, '\n\n>>> Page \03{blue}[[:%s:%s]]\03{default} does not exist! <<<' % (mova, ee))
                    if self.getOption('ignwarn'):
                        if self.user_confirm('Do you want to replace "%s" with wikilink?' % iw.text):
                            return treba, tekst
                    return
            except KeyboardInterrupt:
                raise KeyboardInterrupt
        ...
        if HEREexist:
            ...
            if WikidataID == HEREWikidataID:
                if not HEREredirect:
                    ...
                    if self.getOption('ignwarn'):
                        if (tekst == u""):
                            if self.user_confirm('Do you want to replace "%s" with [[%s]]?' % (iw.text, HEREredirectTitle)):
                                return HEREredirectTitle, tekst
                            if self.user_confirm('Do you want to replace "%s" with [[%s]]?' % (iw.text, HEREredirectTitle[0:1].lower() + HEREredirectTitle[1:])):
                                return HEREredirectTitle[0:1].lower() + HEREredirectTitle[1:], tekst
                            if self.user_confirm('Do you want to replace "%s" with [[%s]]?' % (iw.text, treba)):
                                return treba, tekst
                            ...
                            if self.user_confirm('Do you want to replace "%s" with [[%s]]?' % (iw.text, tekst)):
                                return tekst, tekst
                        else:
                            if self.user_confirm('Do you want to replace "%s" with [[%s|%s]]?' % (iw.text, HEREredirectTitle, tekst)):
                                return HEREredirectTitle, tekst
                            if self.user_confirm('Do you want to replace "%s" with [[%s|%s]]?' % (iw.text, treba, tekst)):
                                return treba, tekst
            else:
            ...
            if tekst == '':
                tekst = treba
            if   self.user_confirm('Do you want to replace "%s" with [[%s|%s]]?' % (iw.text, tranlsatedInto, tekst)):
                return tranlsatedInto, tekst
            if self.user_confirm('Do you want to replace "%s" with [[%s]]?' % (iw.text, tranlsatedInto)):
                return tranlsatedInto, tranlsatedInto
            if self.user_confirm('Do you want to replace "%s" with [[%s]]?' % (iw.text, tranlsatedInto[0:1].lower() + tranlsatedInto[1:])):
    ...
    def getFields(self, pageText, iw):
        lfields = TmplOps.getLuaFields(iw, lfieldnames = ['треба', 'текст', 'мова', 'є', 'nocat'], getFieldPos = False)
    ...
    def analyzeIwItems(self):
        ...
        for item in sorted(self.IwItems.keys()):
            ...
            for request in self.IwItems[item]:                
                ...
                if not (movaEcurrent) in lMovaE:
                    ...
                    if Nrequests > 1:
                        sMovaE += '<br>'
                    sMovaE += '[[:%s:%s]]' % (request['pageToTranslate']['lang'], request['pageToTranslate']['title'])
                    if request['pageToTranslate']['redirect']:
                        sMovaE += ' (→ [[:%s:%s]])' % (request['pageToTranslate']['lang'], request['pageToTranslate']['redirectTitle'])
        ...
        from time import gmtime, strftime
        report = '''== Список неперекладених сторінок ==
...
class ReportByTopicBot(GenBot):
    ...
    def __init__(self):
        ...
        self.generalPages = {'Report': {'title': 'Користувач:PavloChemBot/Неперекладені сторінки',
        ...
        page = pywikibot.Page(site, 'Вікіпедія:Проект:Біологія/Неперекладені статті/Категорії')
        self.topics['Біологія'] = json.loads(page.text)
        self.topics['Біологія']['page'] = 'Вікіпедія:Проект:Біологія/Неперекладені статті'
        
        page = pywikibot.Page(site, 'Користувач:PavloChemBot/Неперекладені сторінки/Хімія/Категорії')
        self.topics['Хімія'] = json.loads(page.text)
        self.topics['Хімія']['page'] = 'Користувач:PavloChemBot/Неперекладені сторінки/Хімія'

        page = pywikibot.Page(site, 'Користувач:PavloChemBot/Неперекладені сторінки/Математика/Категорії')
        self.topics['Математика'] = json.loads(page.text)
        self.topics['Математика']['page'] = 'Користувач:PavloChemBot/Неперекладені сторінки/Математика'
    ...
    def run(self):
        ...
        if  self.getOption('dumpreport'):
            self.iwrobot = IwBot(locargs = ['-ref:Шаблон:Не перекладено', '-report', '-dumpreport'])
            self.iwrobot.run()
            self.updatePages()
        elif self.getOption('loadreport'):
            self.iwrobot = IwBot(locargs = ['-ref:Шаблон:Не перекладено', '-report', '-loadreport'])
            #self.iwrobot.run()
    ...
    def updatePages(self):
        ...
        for line in tmpspl:
            if 'Всього таких статей' in line:
                line += ' Показані лише найбільш поширені в інших розділах Вікімедіа (статей не менше %d).' % self.generalPages['Report']['minNoRequests']
            if '[[#Сторінки, які можливо потребують уваги|потенційні проблеми]]' in line:
                line = line.replace('#Сторінки, які можливо потребують уваги', self.generalPages['Problems'])
            if '| [[d:' in line[:len('| [[d:')]:
    ...
    def topicReport(self, topic):
        ...
        pywikibot.output('Finished getting all pages transcluding iw template after %.1f seconds' % (time.time() - start))
        ...
        pywikibot.output('Finished collecting categories to include into the topic topic after %.1f seconds' % (time.time() - start))
        ...
        pywikibot.output('Finished collecting categories to exclude from the topic after %.1f seconds' % (time.time() - start))

        # Get all required categories
        lcatsreduced = [cat for cat in lcats if not cat in lcatsExclude]
        pywikibot.output('Finished collecting all required categories after %.1f seconds' % (time.time() - start))
        ...
        pywikibot.output('Finished collecting pages in the topic after %.1f seconds' % (time.time() - start))
...
%s</div>''' % (topic, catText, catexcText)
        ...
        if Npages > 1:
            report = 'Оскільки кількість статей більше ніж удвічі перевищує %d, ця сторінка розбита на %d підсторінок:\n\n' % (NitemsPerPage, Npages)
            ...
            report = self.iwrobot.report
            self.iwrobot.IwItems = oldIwItems
        pywikibot.output('Finished preparing list of not translated pages after %.1f seconds' % (time.time() - start))
        ...
        pywikibot.output('Finished preparing list of problems after %.1f seconds' % (time.time() - start))
        ...
        summary = '[[User:PavloChemBot/Iw|автоматичне оновлення]] таблиць'
        self.userPut(page, page.text, text, summary=summary)
        pywikibot.output('Finished putting report after %.1f seconds' % (time.time() - start))

    def findPages(self, catname, catrdepth):
        if catrdepth == 'infinite':
            locargscat = ['-ref:Шаблон:Не перекладено', '-catr:%s' % catname, '-intersect']
        else:
            locargscat = ['-ref:Шаблон:Не перекладено', '-catr:%s' % catname, '-intersect', '-catrdepth:%d' % catrdepth]
        findBot = GenBot(showHelp='iw', locargs = locargscat)
        for page in findBot.generator:
